...
class _AHAF(torch.autograd.Function):
    @staticmethod
    def _forward_old_torch(ctx: FunctionCtx, u, beta, gamma) -> Any:
        ctx.save_for_backward(u, beta, gamma)
        return _AHAF._forward_new_torch(u, beta, gamma)

    @staticmethod
    def _forward_new_torch(u, beta, gamma) -> Any:
    ...
    def forward(*args, **kwargs) -> Any:
        if torch.__version__[0] == '1':
            return _AHAF._forward_old_torch(*args, **kwargs)
        else:
            return _AHAF._forward_new_torch(*args, **kwargs)
