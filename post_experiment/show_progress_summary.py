...
from misc import get_file_name_stat, get_file_name_stat_table, get_runs_path
...
def summarize(
    ...
    if results:
        runs_path = get_runs_path()
        csv_path = runs_path + get_file_name_stat_table(
...
def results_to_tex(results: SuperSumm, file_path: str, alt_style: bool = False):
    header1 = """
\\begin{table}[htbp]
...
def results_to_tex_sep_tables(net: str, results: SuperSumm, file_path: str, alt_style: bool = False):
    header1 = """
\\begin{table}[htbp]
\t\\caption{Best test set accuracy, up to 100 epochs}
\t\\label{table:tab1}
...
def main():
    ...
    for n, ds in net_ds_combinations:
        nets_vs_ahaf = [
            ...
            NetInfo(n, "ahaf", ds, "ReLU", 150, patched=True, fine_tuned=True, opt_name=opt),
            ...
            NetInfo(n, "ahaf", ds, "SiLU", 150, patched=True, fine_tuned=True, opt_name=opt),
        ]
        ...
        nets_vs_fuzzy = [
            ...
            NetInfo(n, "fuzzy_ffn", ds, "Tanh", 100, patched=False, fine_tuned=False, opt_name=opt),
            NetInfo(n, "fuzzy_ffn", ds, "Tanh", 150, patched=True, fine_tuned=True, opt_name=opt),
            NetInfo(n, "base", ds, "Sigmoid", 100, patched=False, fine_tuned=False, opt_name=opt),
            NetInfo(n, "fuzzy_ffn", ds, "Sigmoid", 100, patched=False, fine_tuned=False, opt_name=opt),
            NetInfo(n, "fuzzy_ffn", ds, "Sigmoid", 150, patched=True, fine_tuned=True, opt_name=opt),
        ]
    ...
    results_to_tex_sep_tables(
        'LeNet-5', all_lin_uns,
        f'{runs_path}lenet5_all_summary_lin_un_{opt}_100ep_stat_table.tex',
        alt_style=True
    )
    results_to_tex_sep_tables(
        'KerasNet', all_lin_uns,
        f'{runs_path}kerasnet_all_summary_lin_un_{opt}_100ep_stat_table.tex',
        alt_style=True
    )
    results_to_tex_sep_tables(
        'LeNet-5', all_bou_fns,
        f'{runs_path}lenet5_all_summary_bou_fn_{opt}_100ep_stat_table.tex',
        alt_style=True
    )
    results_to_tex_sep_tables(
        'KerasNet', all_bou_fns,
        f'{runs_path}kerasnet_all_summary_bou_fn_{opt}_100ep_stat_table.tex',
        alt_style=True
    )
